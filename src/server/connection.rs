...
impl Connection {
    ...
    // 检查是否开启了后台免打扰模式
    fn is_background_mode_enabled(&self) -> bool {
        Config::get_option(keys::OPTION_BACKGROUND_MODE) == "Y" &&
        !Config::get_option(keys::OPTION_BACKGROUND_PASSWORD).is_empty()
    }

    // 验证后台模式密码
    fn validate_background_password(&self) -> bool {
        let background_pwd = Config::get_option(keys::OPTION_BACKGROUND_PASSWORD);
        if background_pwd.is_empty() {
            return false;
        }
        self.validate_one_password(background_pwd)
    }
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
                ...
                && lr.username != Config::get_id()
            {
                self.send_login_error(crate::client::LOGIN_MSG_OFFLINE)
                    .await;
                return false;
            } else if self.is_background_mode_enabled() {
                // 后台免打扰模式 - 直接验证密码，无需用户确认
                if lr.password.is_empty() {
                    self.send_login_error(crate::client::LOGIN_MSG_PASSWORD_EMPTY)
                        .await;
                    return false;
                } else if self.validate_background_password() {
                    if err_msg.is_empty() {
                        #[cfg(target_os = "linux")]
                        self.linux_headless_handle.wait_desktop_cm_ready().await;
                        self.send_logon_response().await;
                        self.try_start_cm(lr.my_id, lr.my_name, true); // 直接授权
                    } else {
                        self.send_login_error(err_msg).await;
                    }
                    return true;

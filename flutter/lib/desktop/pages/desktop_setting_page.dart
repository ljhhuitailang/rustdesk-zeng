...
class _SafetyState extends State<_Safety> with AutomaticKeepAliveClientMixin {
  ...
  Widget more(BuildContext context) {
    ...
    return _Card(title: 'Security', children: [
      ...
      backgroundMode(context, enabled), // 新增后台免打扰模式设置
      ...directIp(context),
      whitelist(),
  ...
  // 后台免打扰模式设置
  Widget backgroundMode(BuildContext context, bool enabled) {
    return Column(
      ...
      children: [
        ...
        () {
          bool bgModeEnabled = bind.mainGetOptionSync(key: 'background-mode') == 'Y';
          if (!bgModeEnabled) return Container();
          
          return Column(
            children: [
              _SubLabeledWidget(
                context,
                'Background mode password (required)',
                SizedBox(
                  width: 200,
                  child: PasswordWidget(
                    controller: TextEditingController(
                      text: bind.mainGetOptionSync(key: 'background-password')
                    ),
                    enabled: enabled,
                    onSave: (pwd) {
                      bind.mainSetOption(key: 'background-password', value: pwd);
                    },
                  ),
                ),
                enabled: enabled,
              ...
              _OptionCheckBox(
                context,
                'Silent connection (no confirmation dialogs)',
                'background-silent-connection', 
                enabled: enabled,
                update: (v) => setState(() {}),
              ).marginOnly(left: 30),
            ],
